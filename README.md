...
- 在 GitHub 仓库 Settings -> Secrets and variables -> Actions -> Variables 中设置校验值：
  - `HADOOP_TARBALL_SHA512`（通用包校验值），或
  - `HADOOP_TARBALL_SHA512_AMD64` / `HADOOP_TARBALL_SHA512_ARM64`（多架构专用校验值）。
- 如需豁免上游暂未修复且风险可接受的漏洞，请在仓库根目录维护 `.trivyignore`。
- 推送版本标签触发发布：
...
- 构建并推送多架构镜像（`linux/amd64` + `linux/arm64`）到 GHCR
- 生成 SBOM 与 Provenance 证明
- 使用 Cosign（OIDC 无密钥模式）对镜像摘要签名
- 若缺少可用的 Hadoop 校验值变量，工作流会直接失败
